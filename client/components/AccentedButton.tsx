...
export const AccentedButton: FC<AccentedButtonProps> = ({ color, className, onClick, text, disabled = false, loading }) => {
  return (
    <button
      ...
      } tw-duration-300 ${disabled ? 'tw-cursor-default' : 'tw-cursor-pointer'} tw-rounded-[14px]`}
    >
      {loading ? <Image src={dots_loading} className='tw-my-auto tw-mx-auto !tw-mt-[5px]' /> : <h4 className={`${disabled ? 'tw-text-lightgray tw-opacity-50' : 'tw-text-white'}`}>{text}</h4>}
    </button>
  );
};
